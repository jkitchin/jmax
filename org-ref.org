...
(defun org-ref-clean-bibtex-entry()
  ...
  ;; save key for other uses, e.g. saving a pdf
  (kill-new (reftex-get-bib-field "=key=" (bibtex-parse-entry t)))
  (x-set-selection 'CLIPBOARD (reftex-get-bib-field "=key=" (bibtex-parse-entry t)))
  (occur "[^[:ascii:]]")
  (widen)
